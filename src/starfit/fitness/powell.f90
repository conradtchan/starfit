...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% uobyqb.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  subroutine uobyqb(calfun, n, x, rhobeg, rhoend, iprint, maxfun, npt)
    ...
    real(kind=real64), dimension(n)  :: &
         xbase, xopt, xnew, g, d
    real(kind=real64), dimension(npt,n)  :: &
         xpt
    real(kind=real64), dimension(npt-1)  :: &
         pq
    real(kind=real64), dimension(npt,npt-1) :: &
         pl
    real(kind=real64), dimension(n,n) :: &
         h
    real(kind=real64), dimension(npt) :: &
        vlag, w
    ...
    real(kind=real64) :: &
         ddknew, delta, detrat, diff, distest, dnorm, errtol, estim, &
         evalue, f, fbase, fopt, fsave, ratio, rho, rhosq, sixthm, &
         sum, sumg, sumh, temp, tempa, tworsq, vmax, vquad, wmult

    integer(kind=int64) :: &
         i, ih, ip, iq, iw, j, jswitch, k, knew, kopt, ksave, ktemp, &
         nf, nftest, nnp, nptm
...
70  continue
    ...
    IF (nf <= nnp) THEN
       ...
       IF (jswitch > 0) THEN
          ...
          ! interpolation point that provides diagonal second
          ! derivatives.
       ...
       ! Form the off-diagonal second derivatives of the initial
       ! quadratic model.
    ...
    IF (nf > nnp) THEN
       ...
       ! Pick the shift from XBASE to the next initial interpolation
       ! point that provides off-diagonal second derivatives.
...
80  continue
    ...
    ! Form the gradient of the quadratic model at the trust region
    ! centre.
...
90  continue
    ...
    ! KNEW to -1 if the purpose of the next F will be to improve
    ! conditioning, and also calculate a lower bound on the Hessian
    ! term of the model Q.
...
150 continue
    ...
    ! Update SIXTHM, which is a lower bound on one sixth of the
    ! greatest third derivative of F.
    ...
    ! objective function so far.  Then branch if D is not a trust
    ! region step.
    ...
    ! Replace the interpolation point that has index KNEW by the point
    ! XNEW, and also update the Lagrange functions and the quadratic
    ! model.
...
320 continue
    ...
    IF (knew > 0) THEN
       ...
       ! If ERRTOL is positive, test whether to replace the
       ! interpolation point with index KNEW, using a bound on the
       ...
       ! If the KNEW-th point may be replaced, then pick a D that
       ! gives a large value of the modulus of its Lagrange function
       ! within the trust region.  Here the vector XNEW is used as
    ...
    ! if t is too short to have been tried before.
...
5600 FORMAT (/T5, 'At the return from UOBYQA',  &
  ...
  SUBROUTINE trstep(n, g, h, delta, tol, d, evalue)
    ...
    real(kind=real64), dimension(n) :: &
         gg, td, tn, w, piv, z
    real(kind=real64) :: &
         delsq, dhd, dnorm, dsq, dtg, dtz, gam, gnorm, gsq, hnorm, &
         par, parl, parlest, paru, paruest, phi, phil, phiu, pivksv, &
         pivot, posdef, scale, shfmax, shfmin, shift, slope, sum, &
         tdmin, temp, tempa, tempb, wsq, wwsq, wz, zsq
    INTEGER(kind=int64) :: &
         i, iterc, j, jp, k, kp, kpp, ksav, ksave, nm
    ...
    ! that is similar to H, and put the elements of the Householder
    ! vectors in the lower triangular part of H.  Further, TD and TN
    ! will contain the diagonal and other nonzero elements of the
    ! tridiagonal matrix.
...
170 continue
    ...
    IF (piv(k) >= zero) THEN
       ...
       ! Branch if all the pivots are positive, allowing for the case
       ! when G is zero.
    ...
    ! Set D to a direction of nonpositive curvature of the given
    ! tridiagonal matrix, and thus revise PARLEST.
...
190 continue
    ...
    ! Terminate with D set to a multiple of the current D if the
    ! following test suggests that it suitable to do so.
...
250 continue
    ...
    ! Set EVALUE to the least eigenvalue of the second derivative
    ! matrix if D is a Newton-Raphson step. SHFMAX will be an upper
    ! bound on EVALUE.
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% lagmax.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  SUBROUTINE lagmax(n, g, h, rho, d, v, vmax)
    ...
    real(kind=real64) :: &
         dd, dhd, dlin, dsq, gd, gg, ghg, gnorm, hmax, ratio, &
         scale, sum, sumv, temp, tempa, tempb, tempc, tempd, tempv, &
         vhg, vhv, vhw, vlin, vmu, vnorm, vsq, vv, wcos, whw, wsin, wsq
    INTEGER(kind=int64) :: &
         i, j, k
    ...
    ! Set D to a vector in the subspace spanned by V and HV that
    ! maximizes |(D,HD)|/(D,D), except that we set D=HV if V and HV
    ! are nearly parallel.  The vector that has the name D at label 60
    ! used to be the vector W.
    ...
    ! We now turn our attention to the subspace spanned by G and D. A
    ! multiple of the current D is returned if that choice seems to be
    ! adequate.
    ...
    ! Hence we generate an orthonormal basis of this subspace such
    ! that (D,HV) is negligible or zero, where D and V will be the
    ! basis vectors.
    ...
    ! The final D is a multiple of the current D, V, D+V or D-V. We
    ! make the choice from these possibilities that is optimal.
